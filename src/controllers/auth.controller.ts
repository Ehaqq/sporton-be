...
const JWT_SECRET = process.env.JWT_SECRET || "Sporton321";
...
export const initiateAdmin = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (count > 0) {
            res.status(400).json({message: "We can only have 1 admin user, if you want to create admin user, please delete manually from database"})
            return;
        }
